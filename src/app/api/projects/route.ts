...
const PROJECT_FIELDS =
  'id, kind, reference, title, stage, priority, next_action, due_date, property_city, property_type, type_bien, budget_max, estimated_price_min, estimated_price_max, seller_name, communes, lead_id, active, created_at, updated_at'
...
async function loadContactsByProject(projectIds: string[]): Promise<Map<string, ProjectContact[]>> {
  ...
  const { data: links, error: linksError } = await supabaseAdmin
    ...
    .select('id, contact_id, opportunity_id, buyer_criteria_id, role, is_titulaire')
    .order('created_at', { ascending: true })
    .order('id', { ascending: true })

  if (linksError) {
    console.error('[API /projects] project_contacts error:', linksError)
    return byProject
  }
  ...
  const relevant = (links ?? [])
    .map((link) => ({
      ...
      role: link.role,
      isTitulaire: link.is_titulaire === true,
    }))
    .filter(
      (
        link
      ): link is { projectId: string; contactId: string; role: string; isTitulaire: boolean } =>
  ...
  if (contactsError) {
    console.error('[API /projects] contacts error:', contactsError)
    return byProject
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    const enriched = rows.map((row) => {
      ...
      const city =
        (kind === 'vente' ? row.property_city : (row.communes ?? [])[0]) ?? row.property_city

      const displayTitle = buildProjectTitle({
        titulaireLastNames: contacts
          .filter((contact) => contact.is_titulaire)
          .map((contact) => contact.last_name),
        declaredName: contacts.length === 0 ? row.seller_name : null,
        city,
      })
